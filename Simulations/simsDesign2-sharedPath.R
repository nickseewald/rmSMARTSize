...
###  Test of Formula for Shared-Path Comparisons  ###
### --------------------------------------------- ###
...
gammas <- c(33.5, -0.8, 1.2, -0.8, 0.4, 1.1, 0.1)
lambdas <- c(0.1, -0.4)
...
save(file = here("Results", "simsDesign2-delta3-sharedPath.RData"),
...
for (scenario in 1:nrow(simGrid.delta3)) {
  ...
  if(respFunc.name == "response.oneT"){
    kappa <- qnorm(r1, as.numeric(sum(gammas[1:3])), sigma,
                     lower.tail = FALSE)
    r0 <- pnorm(kappa, sum(gammas[1:2]) - gammas[3], sigma,
                lower.tail = FALSE)
  }

  postID <- paste0(
    "Scenario ", scenario, " of ", nrow(simGrid.delta3), "\n",
    "AR(1) simulation setup\n",
    "Effect size: 0.3\n",
    "Response function:",
    respFunc.name,
    " ", respDir, "\n",
...
gammas <- c(33.5, -0.8, 1.2, -0.8, 0.4, 1.9, 0.1)
lambdas <- c(0.1, -0.4)
...
save(file = here("Results", "simsDesign2-delta5-sharedPath.RData"),
