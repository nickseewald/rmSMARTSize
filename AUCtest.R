...
save(file = "simsDesign2-AUC-delta3-noViol.RData",
     list = c("lambdas", "seed", "times", "spltime"))

sigma <- sigma.r1 <- sigma.r0 <- 6
...
for (i in 1:nrow(simGrid)) {
  ...
  if (corr == 0) {
    gammas <- c(33.5, -0.8, 1.5, -0.8, -1, -0.6, 0.1)
    corr <- c(corr, 0, 0)
  } else if (corr == 0.3) {
    gammas <- c(33.5, -0.8, 0.6, -0.8, 0.4, 0.1, 0.1)
    corr <- c(corr, 0.3 * sigma / sigma.r1, 0.3 * sigma / sigma.r0)
  } else if (corr == 0.6) {
    gammas <- c(33.5, -0.8, 0.6, -0.8, 0.5, 0.4, 0.1)
    corr <- c(corr, 0.6 * sigma / sigma.r1, 0.6 * sigma / sigma.r0)
  } else if (corr == 0.8) {    
    gammas <- c(33.5, -0.8, 0.6, -0.8, 0.5, 0.6, 0.1)
    corr <- c(corr, 0.8 * sigma / sigma.r1, 0.8 * sigma / sigma.r0)
  } else {
    stop(paste("You haven't supplied gammas for correlation", corr[1]))
  }
  ...
  # Simulate
  assign(paste0("d2small.AUC.r", resp * 10, ".exch", corr[1] * 10),
         try(sim(n = 4000, gammas = gammas, lambdas = lambdas, r1 = r1, r0 = r0, times = times, spltime = spltime,
                 alpha = .05, power = .8, delta = 0.3, design = 2, aim = "auc",
                 sigma = sigma, sigma.r1 = sigma.r1, sigma.r0 = sigma.r0,
                 constant.var.time = FALSE, L = c(0, 0, 3, 0, 1, 1, 0),
...
# lambdas <- c(0.1, -0.5)
# 
# save(file = "simsDesign2-AUC-delta5-noViol.RData",
...
# lambdas <- c(0.1, -0.5)
# 
# save(file = "simsDesign2-AUC-delta8-noViol.RData",
#      list = c("sigma", "sigma.r1", "sigma.r0",
#               "gammas", "lambdas", "seed", "times", "spltime"))
# 
# for (i in 1:nrow(simGrid)) {
